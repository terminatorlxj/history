...
(*
let rec mgu eqs = 
  let rec swap_items did eqs = 
    match eqs with
    | [] -> did, []
    | (PTVar v1, PTVar v2)::eqs' -> 
      if v1=v2 then 
        true, snd (swap_items true eqs')
      else 
        let did',swapt = swap_items did eqs' in
        did'||did, (PTVar v1, PTVar v2)::(swapt)
    | (pt, PTVar v)::eqs' -> 
      let _, swapt = swap_items did eqs' in
      true, (PTVar v, pt)::swapt
    | (PTVar v, pt)::eqs' ->
      let did',swapt = swap_items did eqs' in
      did||did', (PTVar v, pt)::swapt
    | (PTInt, PTInt) :: eqs' | (PTFloat, PTFloat)::eqs' | (PTBool, PTBool)::eqs' | (PTUnt, PTUnt)::eqs' -> 
      true, snd (swap_items true eqs')
    | (PTAray pt1, PTAray pt2)::eqs' | (PTLst pt1, PTLst pt2)::eqs' -> 
      true, snd (swap_items true ((pt1,pt2)::eqs'))
    | (PTTuple pts1, PTTuple pts2)::eqs' -> 
      true, snd (swap_items true ((Lists.zip_to_pairs pts1 pts2)@eqs'))
    | (PTRecord str_pts1, PTRecord str_pts2)::eqs' -> 
      if List.length str_pts1 <> List.length str_pts2 then
        raise (Unify_error (PTRecord str_pts1, PTRecord str_pts2))
      else begin
        let rec seconds str_pts1 str_pts2 = 
          match str_pts1, str_pts2 with
          | [],[] -> []
          | (str1, pt1)::str_pts1, (str2, pt2)::str_pts2 -> 
            if str1<>str2 then 
              raise (Unify_error (PTRecord str_pts1, PTRecord str_pts2))
            else 
              (pt1, pt2)::(seconds str_pts1 str_pts2) in
        true, snd (swap_items true ((seconds str_pts1 str_pts2)@eqs'))
      end
    | (PTArrow (pt1, pt2), PTArrow (pt3, pt4))::eqs' -> 
      true, snd (swap_items true ([(pt1, pt3); (pt2, pt4)]@eqs'))
    | (PTConstrs str_opts1, PTConstrs str_opts2)::eqs' -> 
      if List.length str_opts1 <> List.length str_opts2 then
        raise (Unify_error (PTConstrs str_opts1, PTConstrs str_opts2))
      else begin
        let rec seconds str_pts1 str_pts2 = 
          match str_pts1, str_pts2 with
          | [],[] -> []
          | (str1, None)::str_pts1, (str2, None)::str_pts2 -> 
            if str1 <> str2 then
              raise (Unify_error (PTConstrs str_opts1, PTConstrs str_opts2))
            else 
              seconds str_pts1 str_pts2 
          | (str1, Some pt1)::str_pts1, (str2, Some pt2)::str_pts2 -> 
            if str1<>str2 then 
              raise (Unify_error (PTConstrs str_opts1, PTConstrs str_opts2))
            else 
              (pt1, pt2)::(seconds str_pts1 str_pts2) 
          | _ -> raise (Unify_error (PTConstrs str_opts1, PTConstrs str_opts2)) in
        true, snd (swap_items true ((seconds str_opts1 str_opts2)@eqs'))
      end
    | (PTUdt (str1, pts1), PTUdt (str2, pts2))::eqs' -> 
      if str1<>str2 then
        raise (Unify_error (PTUdt (str1, pts1), PTUdt (str2, pts2)))
      else 
        true, snd (swap_items true ((Lists.zip_to_pairs pts1 pts2)@eqs'))
    | (pt1, pt2)::eqs' ->
      if pt1 = pt2 then 
        true, snd (swap_items did eqs')
      else 
        raise (Unify_error (pt1, pt2)) in
  let rec replace_eqs ptv pt eqs nth did = 
    match ptv, eqs with
    | PTVar _, [] -> did, []
    | PTVar v, ((pt1, pt2)::eqs1) -> 
      if nth=0 then 
        let did', replaced = replace_eqs ptv pt eqs1 (-1) did in
        did||did', (pt1, pt2):: replaced
      else
        if (occurs ptv pt1) || (occurs ptv pt2) then 
          true, (replace_ptvar pt1 v pt, replace_ptvar pt2 v pt)::(snd (replace_eqs ptv pt eqs1 (nth-1) true))
        else 
          let did', replaced = replace_eqs ptv pt eqs1 (nth-1) did in
          did'||did, (pt1, pt2)::replaced
    | _ -> did, eqs in
  let modified = ref false in
  let tmp_eqs = ref eqs in
  while !modified do
    (* modified := false; *)
    let tmp_modified, tmp_swapt = swap_items false !tmp_eqs in
    tmp_eqs := tmp_swapt;
    modified := tmp_modified;
    List.iter (fun (pt1, pt2) -> if occurs pt1 pt2 then raise (Unify_error (pt1, pt2)) else ()) !tmp_eqs;  
    (* let replace_modified = ref false in *)
    for i = 0 to List.length !tmp_eqs do
      let ptv, pt = List.nth !tmp_eqs i in
      let tmp_replace_modified, tmp_replaced = replace_eqs ptv pt !tmp_eqs i !modified in
      modified := tmp_replace_modified;
      tmp_eqs := tmp_replaced
    done
  done;
  !tmp_eqs
*)
...
let rec check_pel_type pel env tctx modul moduls = 
  ...
  | PInt i -> let env1 = unify [pel.ptyp; PTInt] modul moduls in (merge_env env1 env modul moduls, tctx)
